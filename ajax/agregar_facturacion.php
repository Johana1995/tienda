...
if (isset($_GET['id']) )//codigo elimina un elemento del array
{
	...
	$sql = "DELETE FROM tmp_venta WHERE producto='".$id."' and sucursal=1 and caja=1";
...
else
{
	...
	{
		$db= DataBase::singleton();
		$sql = "SELECT count(*) AS numrows from tmp_venta WHERE producto='".$id."' and sucursal=1 and caja=1";
		$query = $db->prepare($sql);
		$query->execute();
		...
		{
			$db= DataBase::singleton();
			$sql = "UPDATE tmp_venta set paquete=paquete+'$paquetes', unidad=unidad+'$unidades' WHERE producto='$id' and sucursal=1 and caja=1";
			$query = $db->prepare($sql);
			$query->execute();
		}else
		{
			$db= DataBase::singleton();
			$sql = "INSERT INTO tmp_venta  VALUES (1,1,'$id','$paquetes','$unidades')";
			$query = $db->prepare($sql);
			$query->execute();
		}
	}
...
<table class="table">
	...
	$sql = "select p.id,p.codigo,p.detalle,p.precioPackinVenta as precioPack,p.precioUnidadVenta as precioU,
            				   ...
            				   from producto p,paquete q, tmp_venta t where q.id=p.paquete_id and p.id=t.producto and t.sucursal=1 and caja=1";
	$query = $db->prepare($sql);
	$query->execute();
	...
	<tr>
		...
		<td class='text-right'><?php echo number_format($total_venta_todo,2);?></td>
		<td></td>
	</tr>
